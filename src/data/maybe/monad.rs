use crate::base::function::ConcurrentFn;
use crate::base::value::Value;
use crate::control::context::monad::{Monad, MonadExt};
use crate::data::maybe::{Maybe, MaybeInstance};

impl Monad for MaybeInstance {
    fn bind<A, B, G>(x: Self::Type<A>, g: G) -> Self::Type<B>
    where
        A: Value,
        B: Value,
        G: for<'a> Value<View<'a>: ConcurrentFn<A, Output = Self::Type<B>>>,
    {
        match x {
            Maybe::Just(x) => g.view().call(x),
            Maybe::Nothing => Maybe::Nothing,
        }
    }
}

impl<T> MonadExt for Maybe<T>
where
    T: Value,
{
    type Wrapped = T;
    type Instance = MaybeInstance;
}

#[cfg(test)]
mod tests {
    use crate::base::value::arc;

    use super::*;

    #[test]
    fn test_bind() {
        let m = Maybe::Just(1).bind(&|x| Maybe::Just(x + 1));
        assert_eq!(m, Maybe::Just(2));

        let m: Maybe<i32> = Maybe::Nothing.bind(&|x: i32| Maybe::Just(x + 1));
        assert_eq!(m, Maybe::Nothing);

        let m = Maybe::Just(42).bind(&|x: i32| Maybe::Just(arc(x.to_string())));
        assert_eq!(m, Maybe::Just(arc("42".to_string())));
    }

    #[test]
    fn test_monad_left_identity_law() {
        let g = |x| Maybe::Just(x * 2);
        let lhs = MaybeInstance::ret(3).bind(arc(g));
        let rhs = g(3);
        assert_eq!(lhs, rhs);
    }

    #[test]
    fn test_monad_right_identity_law() {
        let m = Maybe::Just(42);
        let res = m.bind(&MaybeInstance::ret);
        assert_eq!(res, Maybe::Just(42));

        let m: Maybe<i32> = Maybe::Nothing;
        let res = m.bind(&MaybeInstance::ret);
        assert_eq!(res, Maybe::Nothing);
    }

    #[test]
    fn test_monad_associativity_law() {
        let g = |x| Maybe::Just(x + 1);
        let h = |x| Maybe::Just(x * 2);

        let m = Maybe::Just(3);
        let lhs = m.bind(arc(g)).bind(arc(h));
        let rhs = m.bind(arc(move |x| g(x).bind(arc(h))));
        assert_eq!(lhs, rhs);
        assert_eq!(lhs, Maybe::Just(8));

        let m: Maybe<i32> = Maybe::Nothing;
        let lhs = m.bind(arc(g)).bind(arc(h));
        let rhs = m.bind(arc(move |x| g(x).bind(arc(h))));
        assert_eq!(lhs, rhs);
        assert_eq!(lhs, Maybe::Nothing);
    }

    #[test]
    fn test_chained_bind() {
        let m = MaybeInstance::ret(1)
            .bind(&|x| Maybe::Just(x + 1))
            .bind(&|x| Maybe::Just(x * 3));
        assert_eq!(m, Maybe::Just(6));

        let m: Maybe<i32> = Maybe::Nothing.bind(&|x: i32| Maybe::Just(x + 1));
        assert_eq!(m, Maybe::Nothing);

        let m = Maybe::Just(10).bind(&|x| Maybe::Just(x + 5)).bind(&|x| {
            if x > 12 {
                Maybe::Just(x)
            } else {
                Maybe::Nothing
            }
        });
        assert_eq!(m, Maybe::Just(15));
    }
}
